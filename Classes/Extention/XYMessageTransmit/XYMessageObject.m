...
//    XYLog(@"消息转发的第一步：<%@>类没有实现类方法: %@,请尽快处理!", NSStringFromClass([self class]), NSStringFromSelector(sel));
...
//    XYLog(@"消息转发的第一步：<%@>类没有实现实例方法: %@,请尽快处理!", NSStringFromClass([self class]), NSStringFromSelector(sel));
...
- (void)doesNotRecognizeSelector:(SEL)aSelector {
    XYLog(@"确实没有实现方法：【%@】", NSStringFromSelector(aSelector));
}
